...
//			logger.error(e);
	...
	@SuppressWarnings({ "rawtypes" })
	public static String OTPNotification() throws Exception {
		ExtentReporter.HeaderChildNode("Fetching Otp From Notification");
//		waitTime(2000);
//		DriverManager.getAppiumDriver().context("NATIVE_APP");
...
//		waitTime(8000);
//		AndroidDriver driver = (AndroidDriver) DriverManager.getAppiumDriver();
	
		((AndroidDriver) DriverManager.getAppiumDriver()).openNotifications();
		waitTime(3000);
		List<WebElement> allnotifications = DriverManager.getAppiumDriver().findElements(By.xpath("(//*[@resource-id='android:id/message_text'])[1]"));
		...
		Back(1);
		DriverManager.getAppiumDriver().context("WEBVIEW_1");
		return Otp;
	}
...
(By byLocator, String str) throws Exception {
...
//			logger.error(e);
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static void AndroidSwitchToParentWindow() {
		try {
	//		ArrayList<String> window = new ArrayList(getDriver().getWindowHandles());
...
//			logger.error(e);
	...
	public static void clearBackgroundApps() throws IOException {
	...
	public static void switchPlatformToWebParentWindow(String windowID) {
	...
	public static void switchPlatformToAndroid() {
		...
		System.out.println("Platform:"+ getPlatform());
//		new DriverInstance().generateAndroidCapabilities("hrs", "32181JEGR06049", "4723");
...
//		int elementWid=elementSize.getWidth();
	...
	public static void softAssertVerifyText(String acualText , String expectedtext)throws Exception {
		  try {
		   ...
		   if (acualText!=null) {
		    ...
		    logger.info(acualText +" And "+ expectedtext + "  Column Order is  matched");
	...
	public static void validateColorCode(By element , String expectedColorCode, String colorName)throws Exception {
		  try {
			  String actualColorCode="";
		   Wait(200);
		   if (element!=null) {
			   WebElement button = findElement(element);
				  actualColorCode = button.getCssValue("background-color");
				  
		    softAssert.assertEquals(actualColorCode, expectedColorCode, expectedColorCode);
		    logger.info(actualColorCode +" And "+ expectedColorCode + "  Color Code is  matched" + " Color Code Equal To  "+ colorName);
		    ExtentReporter.extentLoggerPass("Pass",actualColorCode +" And "+ expectedColorCode + "  Color Code is  matched" + " Color Code Equal To  "+ colorName);
		   } else {
		    logger.error( actualColorCode +" And "+ expectedColorCode + " Color Code is not displayed OR matched");
		    ExtentReporter.extentLoggerFail("Fail",  expectedColorCode + " Color Code is not displayed OR matched");
		   }
		  } catch (Exception e) {
		   logger.error(e.getMessage());
		   ExtentReporter.extentLoggerFail("testcase", e.getMessage());
	...
	public static void findAndVerifyText(String actualText, String expectedtext)throws Exception {
		if(actualText.equalsIgnoreCase(expectedtext)) {
			softAssert.assertEquals(actualText, expectedtext);
			logger.info(actualText + " is displayed and matched");
			ExtentReporter.extentLoggerPass("Pass",actualText+ " is displayed and matched");
		} else {
			logger.error(actualText +" is not displayed and matched");
			ExtentReporter.extentLoggerFail("Fail", actualText + " is not displayed and matched");
		}
